	...
	$(function() {
		function mmenuInit() {
			...
			if(wi <= '1024') {
				...
				var $icon = $(".hamburger");
				var $menuRoot = $(".mmenu-init").first();

				if (mmenuAPI) {
					$menuRoot.find(".ibihub-mm-close-bar").remove();
					if ($menuRoot.length) {
						var $closeBar = $(
							'<div class="ibihub-mm-close-bar">' +
							'<button type="button" class="ibihub-mm-close" aria-label="Fermer le menu">' +
							'<i class="fa fa-times" aria-hidden="true"></i></button></div>'
						);
						$menuRoot.prepend($closeBar);
						$closeBar.find(".ibihub-mm-close").on("click", function (e) {
							e.preventDefault();
							mmenuAPI.close();
						});
					}

					$(".mmenu-trigger").off("click").on("click", function() {
						mmenuAPI.open();
					});
